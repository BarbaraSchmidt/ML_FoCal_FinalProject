...
def main():
    ...
    adc_coeff = np.polyfit(
        adc_summary["energy_GeV"],
        adc_summary["mean"],
        1
    )

    adc_slope = adc_coeff[0]
    adc_intercept = adc_coeff[1]

    x_adc = np.linspace(
        adc_summary["energy_GeV"].min(),
        adc_summary["energy_GeV"].max(),
        100
    )
    ...
    plt.plot(
        x_adc,
        adc_slope * x_adc + adc_intercept,
        "-",
        label=f"Fit: y={adc_slope:.1f}x+{adc_intercept:.0f}"
    )
    ...
    print(adc_summary)
    print(f"ADC response slope     = {adc_slope:.3f} ADC/GeV")
    print(f"ADC response intercept = {adc_intercept:.3f} ADC")
